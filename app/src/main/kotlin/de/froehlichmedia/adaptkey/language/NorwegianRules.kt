...
/**
 * D-450: the Norwegian Bokmål [LanguageRules] implementation - built via the Language Contribution Guide's
 * own §8 pipeline, from the English Wiktionary's own coverage of Norwegian Bokmål (no native edition exists
 * on kaikki.org - the page literally named "Norwegian" there is a near-empty stub, confirmed directly).
 * Nynorsk (the other official written standard) is a deliberately separate, not-yet-built pack - see
 ...
 * German-specific compounding/inflection grammar with no Norwegian equivalent implemented yet.
 */
object NorwegianRules : LanguageRules {
    ...
    /** Norwegian has no single-word equivalent to German's S-08 "Uhr" convention after a typed time. */
    ...
    /** Norwegian writes decimals with a comma ("3,14"), like every other implemented language here. */
