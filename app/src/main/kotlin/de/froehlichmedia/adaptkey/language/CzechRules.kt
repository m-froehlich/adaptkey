...
/**
 * D-450: the Czech [LanguageRules] implementation - built via the Language Contribution Guide's own §8
 * pipeline, from Czech's own NATIVE Wiktionary edition (Wikislovník, confirmed directly on kaikki.org). Like
 * every other implemented language, only three of the nine hooks are naively fillable:
 ...
 * grammar with no Czech equivalent implemented yet.
 */
object CzechRules : LanguageRules {
    ...
    /** Czech has no single-word equivalent to German's S-08 "Uhr" convention after a typed time. */
    ...
    /** Czech writes decimals with a comma ("3,14"), like every other implemented language here. */
