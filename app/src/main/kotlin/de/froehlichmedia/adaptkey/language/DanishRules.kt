...
/**
 * D-450: the Danish [LanguageRules] implementation - built via the Language Contribution Guide's own §8
 * pipeline, from the English Wiktionary's own coverage of Danish (no native edition exists on kaikki.org).
 ...
 * would reuse a real confusables scan. The remaining six hooks encode German-specific compounding/
 * inflection grammar with no Danish equivalent implemented yet.
 */
object DanishRules : LanguageRules {
    ...
    /** Danish has no single-word equivalent to German's S-08 "Uhr" convention after a typed time. */
    ...
    /** Danish writes decimals with a comma ("3,14"), like every other implemented language here. */
