...
/**
 * Unit tests for the D-143 URL-mode period-key TLD resolution: [UrlLocale.periodAlternatives] always
 * starts with the plain full stop, then orders TLD suggestions per [UrlLocale]'s own doc comment.
 */
class UrlLocaleTest {
    ...
    @Test
    fun `the plain full stop is always the pre-selected first entry`() {
        assertEquals(".", UrlLocale.periodAlternatives(Locale.GERMANY).first())
        assertEquals(".", UrlLocale.periodAlternatives(Locale.US).first())
    }
}
