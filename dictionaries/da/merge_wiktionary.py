# -*- coding: utf-8 -*-
# One-off script: merges the Danish Wiktionary extracts (from extract_wiktionary.py, the English
# Wiktionary's own coverage of Danish - no native edition exists) with the existing dictionaries/da/dict.tsv.
# Same scope discipline, collision rule, case-match safeguard, and append-not-resort convention as
# dictionaries/pl/merge_wiktionary.py - see that file's own comments for the full reasoning behind each
# point. This thinner source still documents noun/adjective inflection reasonably well (see
# extract_wiktionary.py's own module docstring for the real numbers), so this script completes Wortfamilien
# paradigms for all three categories, though expect more gaps than a native-sourced language.
#
