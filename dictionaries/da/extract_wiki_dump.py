# Extracts real word-frequency and bigram counts for Danish from the COMPLETE Danish Wikipedia XML dump
# (dawiki-latest-pages-articles.xml.bz2 - full dump only, per the standing rule since D-445). Directly
...
DUMP = Path(
    ...
    "/scratchpad/dawiki/dawiki-full.xml.bz2"
)
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Fil|Billede):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in DANISH_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
