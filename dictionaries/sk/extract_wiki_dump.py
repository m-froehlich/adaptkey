# Extracts real word-frequency and bigram counts for Slovak from the COMPLETE Slovak Wikipedia XML dump
# (skwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/cs/extract_wiki_dump.py. Slovak's own diacritic letters (á/ä/č/ď/é/í/ĺ/ľ/ň/ó/ô/ŕ/š/ť/ú/ý/ž)
# have no Turkish-style Unicode-casefolding quirk - plain `str.lower()` is correct here.
...
DUMP = Path(
    ...
    "/scratchpad/skwiki/skwiki-full.xml.bz2"
)
...
TOKEN_RE = re.compile(r"[a-záäčďéíĺľňóôŕšťúýž]+(?:-[a-záäčďéíĺľňóôŕšťúýž]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Súbor|Obrázok):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in SLOVAK_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
