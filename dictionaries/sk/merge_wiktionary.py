# -*- coding: utf-8 -*-
# One-off script: merges the Slovak Wiktionary extracts (from extract_wiktionary.py, the English
# Wiktionary's own coverage of Slovak - no native edition exists) with the existing
# dictionaries/sk/dict.tsv. Same scope discipline, collision rule, case-match safeguard, and
# append-not-resort convention as dictionaries/pl/merge_wiktionary.py - see that file's own comments for the
# full reasoning behind each point. This is by far the smallest fallback source checked so far this project
# (5.3MB) - expect thinner overall coverage than every other language, see extract_wiktionary.py's own
# module docstring for its own genuinely different, positional/untagged noun-table data shape.
#
