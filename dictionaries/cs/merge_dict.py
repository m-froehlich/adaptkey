# Merges the real Czech-Wikipedia-dump frequency counts (wiki_dump_freq.tsv, from extract_wiki_dump.py)
# with real POS tags from kaikki.org's NATIVE Czech Wiktionary extract (wiktionary_allpos.tsv, from
...
#
# D-450 note: this native edition's own proper-noun coverage turned out unusually thin (only 7 raw "name"
# entries in the whole file, confirmed directly - a real source characteristic, not a bug; Czech Wikislovník
# evidently does not catalogue proper nouns the way the English Wiktionary's own broader coverage does for
# other languages this project has built).
