# Extracts real word-frequency and bigram counts for Czech from the COMPLETE Czech Wikipedia XML dump
# (cswiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/tr/extract_wiki_dump.py (multiprocessing across this machine's cores for the CPU-heavy
# per-page cleaning/tokenising step, periodic hapax pruning to keep the accumulating Counters bounded over
# millions of pages). Czech's own diacritic letters (á/č/ď/é/ě/í/ň/ó/ř/š/ť/ú/ů/ý/ž) have no Turkish-style
# Unicode-casefolding quirk - plain `str.lower()` is correct here, no special lowering helper needed.
...
DUMP = Path(
    ...
    "/scratchpad/cswiki/cswiki-full.xml.bz2"
)
...
TOKEN_RE = re.compile(r"[a-záčďéěíňóřšťúůýž]+(?:-[a-záčďéěíňóřšťúůýž]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("k", "s", "v", "z", "o", "u", "i", "a")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Soubor|Obrázek):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in CZECH_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
