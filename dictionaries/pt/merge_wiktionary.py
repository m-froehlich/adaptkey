# -*- coding: utf-8 -*-
# One-off script: merges the Portuguese Wiktionary extracts (from extract_wiktionary.py, the native
# Portuguese Wikcionário edition) with the existing dictionaries/pt/dict.tsv. Same scope discipline,
# collision rule, case-match safeguard, and append-not-resort convention as
# dictionaries/fr/merge_wiktionary.py/dictionaries/es/merge_wiktionary.py - see those files' own comments
# for the full reasoning behind each point (identical here, not reinvented).
#
# D-445 (Portuguese-specific scope limit, see extract_wiktionary.py's own module docstring): this script is
# still run for all three categories (nouns/verbs/adjectives) for POS-tagging purposes (add_tag), but the
# NOUN/ADJECTIVE passes generate essentially no new forms and link almost no lemmas, because Portuguese's
# own native Wiktionary edition simply does not document regular noun/adjective inflection as forms[] data
# (confirmed: only 137 of 52,477 noun lemmas and 70 of 18,413 adjective lemmas have any real form beyond a
# non-word hyphenation marker). VERBS are the real, complete Wortfamilien win this round (~6,700 real lemmas
# with full conjugation tables). This is a genuine, confirmed source limitation - not a bug in this script -
# documented explicitly in AdaptKey-Progress.md/AdaptKey-History.md rather than silently passed off as
# complete parity with French/Spanish/Italian/Dutch.
#
...
def add_tag(row, tag):
    if tag not in row["pos"]:
        ...
        row["pos"].add(tag)
    # Mandatory bare-noun safety (Guide step 4 / D-444): Portuguese does not capitalise common nouns (step
    # 8's own decision) - a tag set that is now exactly {NOUN} must never be left bare.
    if row["pos"] == {"NOUN"}:
        row["pos"].add("OTHER")
