# Merges the real Portuguese-Wikipedia-dump frequency counts (wiki_dump_freq.tsv, from
# extract_wiki_dump.py) with real POS tags from kaikki.org's NATIVE Portuguese Wiktionary extract
# (wiktionary_allpos.tsv, from extract_wiktionary.py - the same native-edition extraction step 4's own
# Wortfamilien completion also reads) into the initial dict.tsv - one shared native-Wiktionary source
# feeding both this step and the word-family completion that runs after it, per the Language Contribution
# Guide's own step 4 note against a second, separate download/parse. Directly modelled on
# dictionaries/fr/merge_dict.py/dictionaries/es/merge_dict.py.
...
# PREPOSITION/PROPER_NOUN/OTHER only).
POS_MAP = {
    "noun": "NOUN",
    "verb": "VERB",
    "adj": "ADJECTIVE",
...
# Step-2-style batched noise removal (Language Contribution Guide §8): a word not recognised by kaikki at
# all is real noise disproportionately often when it is *also* a common English word - Portuguese Wikipedia
# legitimately quotes English titles/technical terms/proper nouns verbatim, and this project's own bundled
# en/dict.tsv is a ready-made, already-reviewed reference for "common English word" (>=100 is comfortably
# above one-off/rare-tail English entries), the identical French/Spanish precedent.
EN_COMMON_MIN_FREQ = 100

# Rank-1 raw dump count is rescaled to ~1,000,000, matching German's own dict.tsv max order of magnitude
# (1,004,234) - see the Language Contribution Guide's step 1 "frequency-scale calibration".
TARGET_TOP = 1_000_000

# A word not found in kaikki (no real Portuguese Wiktionary entry under this exact string) is still kept,
# tagged OTHER, once its own real corpus count clears this floor - low enough to keep genuine words
# Wiktionary just doesn't happen to have a page for, high enough that one-off tokeniser/markup noise mostly
# does not - the identical French/Spanish floor, now against a full (not capped) dump's own larger absolute
# counts.
...
def resolve_tags(kaikki_cats: set) -> str:
    ...
    # kaikki never saw this exact string used any other way.
    if "PROPER_NOUN" in cats and len(cats) > 1:
        cats.discard("PROPER_NOUN")
    # Mandatory bare-noun safety check (Language Contribution Guide step 4, D-444): Portuguese does not
    # capitalise common nouns (§6 rule 3 is not language-gated - see spec §6's D-441 addendum) - pair a
    # bare NOUN with OTHER to keep the real NOUN signal for A-05 while landing on rule 5's "ambiguous, no
    # automatic correction" outcome instead.
    if cats == {"NOUN"}:
        cats.add("OTHER")
    if not cats:
        return "OTHER"
