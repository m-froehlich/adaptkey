# -*- coding: utf-8 -*-
# Einmal-Skript: fasst die spanischen Wiktionary-Extrakte (aus extract_wiktionary.py, der nativen
# spanischen Wiktionary-Ausgabe) mit dem bestehenden dictionaries/es/dict.tsv zusammen. Gleiche
# Scope-Disziplin, Kollisionsregel, Case-Match-Absicherung und Anhaenge-statt-Neusortieren-Konvention wie
# dictionaries/el/merge_wiktionary.py/dictionaries/fr/merge_wiktionary.py - siehe deren eigenen Kommentar
# fuer die volle Begruendung jedes einzelnen Punkts (identisch uebernommen, nicht neu erfunden).
#
...
#     PROPER_NOUN wuerde sie sonst zwingend grossschreiben - siehe die sol/paz/victoria-Faelle, die dieses
#     Projekt fuer Spanisch bereits einmal im Schritt davor gefunden hat).
#  2. NACH jeder Tag-Vergabe (add_tag) wird sofort geprueft, ob die resultierende Tag-Menge exakt {NOUN}
#     ist - wenn ja, wird OTHER direkt mitvergeben. Spanisch schreibt gewoehnliche Nomen nicht gross
...
def process_proper_nouns(words, by_lower, all_pos, stats):
    ...
    # would carry, since PartOfSpeech has no category for those at all. Real, found example: Spanish "les"
    # (a pronoun, "to them") also has a genuine but rare "name" (surname) Wiktionary entry - dict.tsv's own prior tag
