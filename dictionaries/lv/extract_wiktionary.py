...
# ENGLISH Wiktionary's own coverage of Latvian (`kaikki.org/dictionary/Latvian/kaikki.org-dictionary-
# Latvian.jsonl.gz`, 16.5MB) - Latvian has NO native Wiktionary edition on kaikki.org (confirmed directly).
#
# D-450 checks done before writing this, not assumed: standard, fully-tagged forms[] shape, no positional/
# untagged oddity (Slovak) and no pitch-accent notation (Lithuanian, this project's own neighbouring
# language - see dictionaries/lt/extract_wiktionary.py) - confirmed directly against a real noun ("abats") and
# verb ("strādāt"): both document plain, correctly-spelled forms directly usable as-is.
#
# Quelle: https://kaikki.org/dictionary/Latvian/kaikki.org-dictionary-Latvian.jsonl.gz
...
LATVIAN_LETTERS = set("abcdefghijklmnopqrstuvwxyzāčēģīķļņšūž")
VALID_FORM_RE = re.compile(r"^[a-zāčēģīķļņšūž]+(-[a-zāčēģīķļņšūž]+)*$")
...
# D-450 REAL CALIBRATION BUG, found via the mandatory ratio sanity check (verb ratio 16.16x, far beyond the
# sane range): Latvian's own verb-conjugation table includes the row-header PRONOUN SUBJECT as its own,
# genuinely tagged forms[] entry (e.g. `{"form": "viņš", "tags": ["indicative", "singular", "third-person"]}`
# on literally every verb) - "viņš" ("he", freq 74,031) was getting linked as a spurious "form" of dozens of
# unrelated, mostly rare verbs, since a real Latvian verb inflection can never literally equal one of these
# closed-class pronoun strings. Excluded by literal value, not by tag (the tags alone are indistinguishable
# from a real personal-conjugation form's own tags).
PRONOUN_SUBJECT_LABELS = {"es", "tu", "viņš", "viņa", "mēs", "jūs", "viņi", "viņas"}
...
def usable_forms(entry, word):
    ...
    for f in entry.get("forms", []):
        ...
        if not raw_form or raw_form in ("-", "—"):
            continue
        if raw_form.strip().lower() in PRONOUN_SUBJECT_LABELS:
            continue
        tags = set(f.get("tags", []))
...
if __name__ == "__main__":
    in_path = sys.argv[1] if len(sys.argv) > 1 else "lv-extract.jsonl.gz"
