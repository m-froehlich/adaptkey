# Extracts real word-frequency and bigram counts for Latvian from the COMPLETE Latvian Wikipedia XML dump
# (lvwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/cs/extract_wiki_dump.py. Latvian's own diacritic letters (ā/č/ē/ģ/ī/ķ/ļ/ņ/š/ū/ž) have no
# Turkish-style Unicode-casefolding quirk - plain `str.lower()` is correct here.
...
DUMP = Path(
    ...
    "/scratchpad/lvwiki/lvwiki-full.xml.bz2"
)
...
TOKEN_RE = re.compile(r"[a-zāčēģīķļņšūž]+(?:-[a-zāčēģīķļņšūž]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("a", "i", "o", "u")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Attēls):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in LATVIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
