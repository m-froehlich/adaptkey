# Merges the real Swedish-Wikipedia-dump frequency counts (wiki_dump_freq.tsv, from extract_wiki_dump.py)
# with real POS tags from kaikki.org's own English-Wiktionary-coverage-of-Swedish extract
# (wiktionary_allpos.tsv, from extract_wiktionary.py - Swedish has no native Wiktionary edition on kaikki,
# confirmed directly, so this is the correct fallback source per the Guide's own step 3 - expect thinner
# data than a native-sourced language) into the initial dict.tsv - one shared source feeding both this step
# and the word-family completion that runs after it. Directly modelled on dictionaries/pl/merge_dict.py.
...
def resolve_tags(kaikki_cats: set) -> str:
    ...
    # Mandatory bare-noun safety check (Language Contribution Guide step 4, D-444): Swedish does not
    # capitalise ordinary common nouns the way German does, so the same safety pairing applies.
    if cats == {"NOUN"}:
        cats.add("OTHER")
    if not cats:
        return "OTHER"
