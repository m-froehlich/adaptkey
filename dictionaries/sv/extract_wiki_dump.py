# Extracts real word-frequency and bigram counts for Swedish from the COMPLETE Swedish Wikipedia XML dump
# (svwiki-latest-pages-articles.xml.bz2, the single combined dump - full dump only, per the standing rule
# established for every language since D-445). Directly modelled on
# dictionaries/pl/extract_wiki_dump.py (multiprocessing across this machine's 6 physical/12 logical cores,
# periodic hapax pruning to keep the accumulating Counters bounded over millions of pages).
...
DUMP = Path(
    ...
    "/scratchpad/svwiki/svwiki-full.xml.bz2"
)
...
TOKEN_RE = re.compile(r"[a-zåäö]+(?:-[a-zåäö]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("i",)
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Fil|Bild):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in SWEDISH_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
