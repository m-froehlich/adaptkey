# Extracts real word-frequency and bigram counts for Polish from the COMPLETE Polish Wikipedia XML dump
# (plwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl). Directly modelled on
# dictionaries/pt/extract_wiki_dump.py (see that file's own module docstring for the two real additions
...
# bounded over millions of pages). Streams the bz2-compressed XML directly rather than decompressing to disk
# first.
...
DUMP = Path(
    ...
    "/scratchpad/plwiki/plwiki-full.xml.bz2"
)
BASE = Path(__file__).parent
FREQ_OUT = BASE / "wiki_dump_freq.tsv"
BIGRAM_OUT = BASE / "wiki_dump_bigram.tsv"

# Polish alphabet plus its own nine diacritic letters (ą ć ę ł ń ó ś ź ż).
POLISH_LETTERS = set("abcdefghijklmnopqrstuvwxyząćęłńóśźż")
TOKEN_RE = re.compile(r"[a-ząćęłńóśźż]+(?:-[a-ząćęłńóśźż]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("a", "i", "o", "u", "w", "z")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Plik|Grafika):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in POLISH_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
