# -*- coding: utf-8 -*-
# One-off script: merges the Hungarian Wiktionary extracts (from extract_wiktionary.py, the English
# Wiktionary's own coverage of Hungarian - no native edition exists) with the existing
# dictionaries/hu/dict.tsv. Same scope discipline, collision rule, case-match safeguard, and
# append-not-resort convention as dictionaries/pl/merge_wiktionary.py - see that file's own comments for the
# full reasoning behind each point. See extract_wiktionary.py's own module docstring for the two real,
# per-language findings this round (the possessive-suffix paradigm cap, applied consistently with Finnish's
# own precedent, and the "informal" tag meaning a real grammatical T-V distinction here, not slang register).
#
