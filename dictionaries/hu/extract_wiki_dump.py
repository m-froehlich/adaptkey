# Extracts real word-frequency and bigram counts for Hungarian from the COMPLETE Hungarian Wikipedia XML
# dump (huwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the
# same "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/cs/extract_wiki_dump.py. Hungarian's own diacritic letters (á/é/í/ó/ö/ő/ú/ü/ű) have no
...
DUMP = Path(
    ...
    "/scratchpad/huwiki/huwiki-full.xml.bz2"
)
...
TOKEN_RE = re.compile(r"[a-záéíóöőúüű]+(?:-[a-záéíóöőúüű]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("a", "s", "ő")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Fájl|Kép):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in HUNGARIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
