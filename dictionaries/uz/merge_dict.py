# Merges the real Uzbek-Wikipedia-dump frequency counts (wiki_dump_freq.tsv, from extract_wiki_dump.py) with
# POS tags from the ENGLISH-Wiktionary-coverage fallback (wiktionary_allpos.tsv, from extract_wiktionary.py -
# Uzbek has no native Wiktionary edition, see that script's own module docstring) into the initial dict.tsv.
# Directly modelled on dictionaries/az/merge_dict.py.
#
# D-450-followup: unlike Turkish's/Azerbaijani's own postpositional gap, this source carries a real, clean
# `postp` tag - already folded into `extract_wiktionary.py`'s own `prepositions` set (mapped through as
# `pos` value `"postp"` in `wiktionary_allpos.tsv`), so `POS_MAP` below maps it to `PREPOSITION` alongside
# `prep`/`prep_phrase`, no exception-list judgement call needed this time.
