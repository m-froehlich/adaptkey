# -*- coding: utf-8 -*-
# One-off script: merges the Finnish Wiktionary extracts (from extract_wiktionary.py, the English
# Wiktionary's own coverage of Finnish - no native edition exists) with the existing
# dictionaries/fi/dict.tsv. Same scope discipline, collision rule, case-match safeguard, and
# append-not-resort convention as dictionaries/pl/merge_wiktionary.py - see that file's own comments for the
# full reasoning behind each point. This particular fallback source is unusually rich (see
# extract_wiktionary.py's own module docstring for the real numbers - near-total noun/adjective coverage),
# so this script completes real Wortfamilien paradigms for all three categories, though the Guide's own
# standing "expect more gaps than a native-sourced language" caveat still applies.
#
