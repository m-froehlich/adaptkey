# Merges the real Finnish-Wikipedia-dump frequency counts (wiki_dump_freq.tsv, from extract_wiki_dump.py)
# with POS tags from the English Wiktionary's own coverage of Finnish (wiktionary_allpos.tsv, from
# extract_wiktionary.py - no native Wiktionary edition exists for Finnish, confirmed directly) into the
# initial dict.tsv - one shared source feeding both this step and the word-family completion that runs
# after it. Directly modelled on dictionaries/pl/merge_dict.py.
...
def resolve_tags(kaikki_cats: set) -> str:
    ...
    # Mandatory bare-noun safety check (Language Contribution Guide step 4, D-444): Finnish does not
