# Extracts real word-frequency and bigram counts for Finnish from the COMPLETE Finnish Wikipedia XML dump
# (fiwiki-latest-pages-articles.xml.bz2 - full dump only, per the standing rule since D-445). Directly
# modelled on dictionaries/pl/extract_wiki_dump.py (multiprocessing, periodic hapax pruning).
...
DUMP = Path(
    ...
    "/scratchpad/fiwiki/fiwiki-full.xml.bz2"
)
BASE = Path(__file__).parent
FREQ_OUT = BASE / "wiki_dump_freq.tsv"
BIGRAM_OUT = BASE / "wiki_dump_bigram.tsv"

# Standard Latin a-z plus Finnish's own ä/ö, and å (loanword/Swedish-minority-name letter, part of the
# official Finnish alphabet's last position).
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Tiedosto|Kuva):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in FINNISH_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
