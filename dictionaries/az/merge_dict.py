# Merges the real Azerbaijani-Wikipedia-dump frequency counts (wiki_dump_freq.tsv, from extract_wiki_dump.py)
# with POS tags from the ENGLISH-Wiktionary-coverage fallback (wiktionary_allpos.tsv, from
# extract_wiktionary.py - Azerbaijani has no native Wiktionary edition, see that script's own module
# docstring) into the initial dict.tsv. Directly modelled on dictionaries/uk/merge_dict.py.
#
# D-450-followup REAL FINDING: like Turkish (D-449), Azerbaijani is postpositional, not prepositional - this
# fallback source tagged only 3 words `pos=="prep"/"prep_phrase"` in the whole file (vs. thousands of real
# nouns/verbs/adjectives), the same near-zero-closed-class shape Turkish's own native edition showed. No
# curated postposition exception list was built this round either, for the identical reason Turkish's own
# merge_dict.py documents: confidently deciding which candidate postpositions also deserve PREPOSITION
# without native Azerbaijani fluency was judged too risky.
