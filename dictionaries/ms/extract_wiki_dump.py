# Extracts real word-frequency and bigram counts for Malay from the COMPLETE Malay Wikipedia XML dump
# (mswiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/cs/extract_wiki_dump.py. Malay's own alphabet is the plain 26-letter Latin set with no
...
DUMP = Path(
    ...
    "/scratchpad/mswiki/mswiki-full.xml.bz2"
)
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Fail|Imej):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in MALAY_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
