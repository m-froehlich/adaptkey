# Merges the real Italian-Wikipedia-dump frequency counts (wiki_dump_freq.tsv, from extract_wiki_dump.py)
# with real POS tags from kaikki.org's NATIVE Italian Wiktionary extract (wiktionary_allpos.tsv, from
...
# dictionaries/fr/merge_dict.py/dictionaries/pt/merge_dict.py.
...
def resolve_tags(kaikki_cats: set) -> str:
    ...
    # Mandatory bare-noun safety check (Language Contribution Guide step 4, D-444): Italian does not
    # capitalise common nouns (step 8's own decision).
    if cats == {"NOUN"}:
        cats.add("OTHER")
    if not cats:
        return "OTHER"
