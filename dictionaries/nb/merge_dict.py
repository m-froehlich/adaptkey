# Merges the real Norwegian Bokmål-Wikipedia-dump frequency counts (wiki_dump_freq.tsv, from
# extract_wiki_dump.py) with POS tags from the English Wiktionary's own coverage of Norwegian Bokmål
# (wiktionary_allpos.tsv, from extract_wiktionary.py - no native edition exists for this language, confirmed
# directly) into the initial dict.tsv - one shared source feeding both this step and the word-family
# completion that runs after it. Directly modelled on dictionaries/pl/merge_dict.py.
...
def resolve_tags(kaikki_cats: set) -> str:
    ...
    # Mandatory bare-noun safety check (Language Contribution Guide step 4, D-444): Norwegian Bokmål DOES capitalise
    # some proper nouns like every language, but ordinary common nouns are lower-case as usual - not
    # capitalised the way German does, so the same safety pairing applies.
    if cats == {"NOUN"}:
        cats.add("OTHER")
    if not cats:
        return "OTHER"
