# Extracts real word-frequency and bigram counts for Norwegian Bokmål from the COMPLETE Norwegian
# (Bokmål) Wikipedia XML dump (nowiki-latest-pages-articles.xml.bz2 - "no" is Wikipedia's own historical
# dump-naming convention for the Bokmål edition; kaikki's own lang_code for this variety is "nb", used
# throughout this project's own Language enum/dict.tsv/directory naming instead, see D-450's own note on the
# mismatch) - the single combined dump, full dump only, per the standing rule since D-445. Directly modelled
# on dictionaries/pl/extract_wiki_dump.py (multiprocessing, periodic hapax pruning).
...
DUMP = Path(
    ...
    "/scratchpad/nowiki/nowiki-full.xml.bz2"
)
...
TOKEN_RE = re.compile(r"[a-zæøå]+(?:-[a-zæøå]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("i", "å")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Fil|Bilde):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in NORWEGIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
