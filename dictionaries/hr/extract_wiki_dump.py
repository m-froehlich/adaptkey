# Extracts real word-frequency and bigram counts for Croatian from the COMPLETE Croatian Wikipedia XML dump
# (hrwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/cs/extract_wiki_dump.py. Croatian's own diacritic letters (č/ć/đ/š/ž) have no Turkish-style
# Unicode-casefolding quirk - plain `str.lower()` is correct here. Croatian is exclusively written in Latin
# script (unlike Serbian, which this project builds separately in Cyrillic - see dictionaries/sr/'s own
# module docstring for that decision).
...
DUMP = Path(
    ...
    "/scratchpad/hrwiki/hrwiki-full.xml.bz2"
)
BASE = Path(__file__).parent
FREQ_OUT = BASE / "wiki_dump_freq.tsv"
BIGRAM_OUT = BASE / "wiki_dump_bigram.tsv"

CROATIAN_LETTERS = set("abcčćdđefghijklmnoprsštuvzž")
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in CROATIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
