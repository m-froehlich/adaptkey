...
# is comfortably above one-off/rare-tail English entries). A manual sample of the low-confidence band
# confirmed this catches real contamination (countries/reviews/genocide/tourist/...) while leaving
# genuine rare French vocabulary (écobuage/hémiptères/inexactitudes/...) untouched - not exhaustive, but
# a real, targeted pass rather than skipping step 2 entirely.
...
# A word not found in kaikki (no real French Wiktionary entry under this exact string) is still kept,
...
# (typically count 1-a handful in a 306k-article sample) mostly does not.
...
def resolve_tags(kaikki_cats: set) -> str:
    ...
    # kaikki never saw this exact string used any other way (a real pierre/jean-shaped collision with an
    ...
    # A bare NOUN would auto-capitalise in French too (§6 rule 3 is not language-gated - see spec §6's
