# Extracts real word-frequency and bigram counts for Swahili from the COMPLETE Swahili Wikipedia XML dump
# (swwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/id/extract_wiki_dump.py. Swahili's own alphabet is the plain 26-letter Latin set with no
# diacritics at all (confirmed directly against the real Wiktionary source) - no casefolding quirk either.
...
DUMP = Path(
    ...
    "/scratchpad/swwiki/swwiki-full.xml.bz2"
)
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Faili|Picha):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in SWAHILI_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
