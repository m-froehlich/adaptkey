# Extracts real word-frequency and bigram counts for Tagalog from the COMPLETE Tagalog Wikipedia XML dump
# (tlwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/sw/extract_wiki_dump.py. Tagalog's own everyday alphabet (the modern Filipino orthography) is
# the plain 26-letter Latin set with no diacritics (confirmed directly against the real Wiktionary source;
# the historical Baybayin script is a separate, non-Latin alternative, not the everyday writing system) - no
# casefolding quirk either.
...
DUMP = Path(
    ...
    "/scratchpad/tlwiki/tlwiki-full.xml.bz2"
)
BASE = Path(__file__).parent
FREQ_OUT = BASE / "wiki_dump_freq.tsv"
BIGRAM_OUT = BASE / "wiki_dump_bigram.tsv"

TAGALOG_LETTERS = set("abcdefghijklmnopqrstuvwxyz")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Larawan):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in TAGALOG_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
