...
# ENGLISH Wiktionary's own coverage of Tagalog (`kaikki.org/dictionary/Tagalog/kaikki.org-dictionary-
# Tagalog.jsonl.gz`, 14.7MB) - Tagalog has NO native Wiktionary edition on kaikki.org (confirmed directly).
#
# D-450 REAL FINDINGS, confirmed by direct inspection: (1) Tagalog entries carry an alternative BAYBAYIN-
# script spelling (the historical pre-colonial script, tagged `["Baybayin"]`, e.g. "ᜊᜐᜒᜈ᜔" for "basin") -
# excluded explicitly (the same defense-in-depth pattern already used for Malay's own Jawi tag), even though
# Baybayin's own characters already fall outside `VALID_FORM_RE` regardless. (2) the `"canonical"` headword
# form carries a stress-accent mark (e.g. "basín" for "basin") - already excluded via the existing
# `EXCLUDE_FORM_TAGS`, and confirmed directly that the REAL conjugated forms (completive/progressive/
# contemplative - Tagalog's own real aspect-system terminology, not past/present/future) are all clean,
# accent-free spellings, so no `links`-based recovery (the Serbo-Croatian/Lithuanian fix) was needed here.
#
# Quelle: https://kaikki.org/dictionary/Tagalog/kaikki.org-dictionary-Tagalog.jsonl.gz
...
# "Baybayin" excluded explicitly (see module docstring) - defense in depth alongside VALID_FORM_RE.
EXCLUDE_FORM_TAGS = {"table-tags", "inflection-template", "canonical", "error-unrecognized-form", "error-unknown-tag", "class", "Baybayin"}
...
if __name__ == "__main__":
    in_path = sys.argv[1] if len(sys.argv) > 1 else "tl-extract.jsonl.gz"
