# Extracts real word-frequency and bigram counts for Lithuanian from the COMPLETE Lithuanian Wikipedia XML
# dump (ltwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the
# same "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/cs/extract_wiki_dump.py. Lithuanian's own diacritic letters (ą/č/ę/ė/į/š/ų/ū/ž) have no
# Turkish-style Unicode-casefolding quirk - plain `str.lower()` is correct here. (Note: the pitch-accent
# notation issue found in this language's own Wiktionary source - see extract_wiktionary.py's own module
# docstring - is specific to that dictionary source, not to real Wikipedia running text, so it does not
# affect this script.)
...
DUMP = Path(
    ...
    "/scratchpad/ltwiki/ltwiki-full.xml.bz2"
)
...
TOKEN_RE = re.compile(r"[a-ząčęėįšųūž]+(?:-[a-ząčęėįšųūž]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("i", "o")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Vaizdas):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in LITHUANIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
