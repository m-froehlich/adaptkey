# Extracts real word-frequency and bigram counts for Estonian from the COMPLETE Estonian Wikipedia XML dump
# (etwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/fi/extract_wiki_dump.py (Estonian's own closest linguistic relative in this project).
# Estonian's own diacritic letters (õ/ä/ö/ü) have no Turkish-style Unicode-casefolding quirk - plain
# `str.lower()` is correct here.
...
DUMP = Path(
    ...
    "/scratchpad/etwiki/etwiki-full.xml.bz2"
)
BASE = Path(__file__).parent
FREQ_OUT = BASE / "wiki_dump_freq.tsv"
BIGRAM_OUT = BASE / "wiki_dump_bigram.tsv"

ESTONIAN_LETTERS = set("abcdefghijklmnopqrstuvwxyzõäöü")
TOKEN_RE = re.compile(r"[a-zõäöü]+(?:-[a-zõäöü]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ()
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Pilt):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in ESTONIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
