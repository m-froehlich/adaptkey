# Extracts real word-frequency and bigram counts for Dutch from the COMPLETE Dutch Wikipedia XML dump
# (nlwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split) - per
...
# for the two real additions needed at full-dump scale: multiprocessing across this machine's 6 physical/12
# logical cores for the CPU-heavy per-page cleaning/tokenising step, and periodic hapax pruning to keep the
# accumulating Counters bounded over millions of pages). Streams the bz2-compressed XML directly rather than
...
DUMP = Path(
    ...
    "/scratchpad/nlwiki/nlwiki-full.xml.bz2"
)
BASE = Path(__file__).parent
FREQ_OUT = BASE / "wiki_dump_freq.tsv"
BIGRAM_OUT = BASE / "wiki_dump_bigram.tsv"

# Dutch alphabet plus its real diaeresis/accent loanword variants.
DUTCH_LETTERS = set("abcdefghijklmnopqrstuvwxyzëïöüéè")
TOKEN_RE = re.compile(r"[a-zëïöüéè]+(?:-[a-zëïöüéè]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("u",)
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Bestand|Afbeelding):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in DUTCH_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
