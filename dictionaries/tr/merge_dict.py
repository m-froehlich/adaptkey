# Merges the real Turkish-Wikipedia-dump frequency counts (wiki_dump_freq.tsv, from extract_wiki_dump.py)
# with real POS tags from kaikki.org's NATIVE Turkish Wiktionary extract (wiktionary_allpos.tsv, from
# extract_wiktionary.py) into the initial dict.tsv - one shared native-Wiktionary source feeding both this
# step and the word-family completion that runs after it. Directly modelled on
# dictionaries/pl/merge_dict.py.
#
# D-449: Turkish is a postpositional, not prepositional, language - this Wiktionary edition tags its own
# postpositions inconsistently across "particle"/"adv"/"conj"/"noun"/"adj" depending on the specific word's
# other senses, with no dedicated closed-class tag the way "prep"/"prep_phrase" cleanly maps for every other
# language this project has built. A small hand-curated exception list was considered and deliberately not
# built this round - several of the best-known postpositions (`sonra`/`doğru`/`karşı`/`dolayı`) already carry
# real, competing noun/adjective/adverb senses in this same data, and confidently deciding which of those
# should ALSO gain PREPOSITION without native Turkish fluency was judged too risky, the same reasoning that
# ruled out a curated list for Portuguese's own "-ão" ambiguity. See the Language Contribution Guide's own
# preposition-tagging step for the general case and this note for Turkish's own real exception.
