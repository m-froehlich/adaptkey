# Extracts real word-frequency and bigram counts for Serbian from the COMPLETE Serbian Wikipedia XML dump
# (srwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr).
#
# D-450 SCRIPT DECISION, directly measured rather than assumed: Serbian is officially bi-scriptural (Cyrillic
# constitutionally primary, Latin also in wide everyday use), a real, consequential choice unlike Croatian/
# Bosnian's own unambiguous Latin-only convention. Checked directly against a real 500-page sample of this
# very dump's own raw wikitext before deciding: 839,282 Cyrillic letters vs. only 151,666 Latin letters
# (roughly 5.5:1) - confirming Serbian Wikipedia's own raw corpus content is overwhelmingly Cyrillic (the
# residual Latin count is consistent with foreign names/citations/external links, not running Serbian text).
# This project's own Serbian pack is therefore built in CYRILLIC script - matching both the dominant script
# of the actual source corpus (far more usable word-frequency data than a Latin-only filter would recover)
# and Serbia's own official constitutional script. The shared `dictionaries/sh/` Serbo-Croatian Wiktionary
# extractor already supports emitting Cyrillic-tagged forms for exactly this reason (see its own module
# docstring).
...
DUMP = Path(
    "C:/Users/MFROEH~1/AppData/Local/Temp/claude/D--workspace-ai/e7bb1ec8-982b-4dfe-8f0f-a0b1aae7431b"
    "/scratchpad/srwiki/srwiki-full.xml.bz2"
)
BASE = Path(__file__).parent
FREQ_OUT = BASE / "wiki_dump_freq.tsv"
BIGRAM_OUT = BASE / "wiki_dump_bigram.tsv"

SERBIAN_LETTERS = set("абвгдђежзијклљмнњопрстћуфхцчџш")
TOKEN_RE = re.compile(r"[абвгдђежзијклљмнњопрстћуфхцчџш]+(?:-[абвгдђежзијклљмнњопрстћуфхцчџш]+)*|['’]", re.IGNORECASE | re.UNICODE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("а", "и", "у", "о", "с")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Датотека|Слика|Slika):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in SERBIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
