...
# ENGLISH Wiktionary's own coverage of Romanian (`kaikki.org/dictionary/Romanian/kaikki.org-dictionary-
# Romanian.jsonl.gz`, 28.5MB) - Romanian has NO native Wiktionary edition on kaikki.org (confirmed directly).
#
# D-450 SCRIPT CHECK, per direct inspection before assuming: modern comma-below Ș/Ț (U+0218/U+0219,
# U+021A/U+021B) is this source's own standard - confirmed directly (25,970 real words use it; only a single
# stray legacy cedilla-Ţ character-page entry uses the older Ş/Ţ cedilla forms, negligible, not worth
# special-casing).
#
# D-450 MULTI-WORD FORM CHECK: Romanian's own infinitive is written with a separate leading marker word ("a
# abate" - "to abate"), and its subjunctive/negative-imperative moods use their own leading marker words too
# ("să abat" - subjunctive marker "să"; "nu abate" - negative-imperative marker "nu") - the same marker-first
# analytic pattern already confirmed for every Nordic language and Turkish this project has built. The same
# "reject any whitespace-containing form outright" rule applies unmodified, no `last_token()` recovery
# attempted (the infinitive marker "a abate" is lost this way, but the bare lemma "abate" itself is always
# the dictionary headword already, so no real coverage is lost by this).
#
# Quelle: https://kaikki.org/dictionary/Romanian/kaikki.org-dictionary-Romanian.jsonl.gz
...
VALID_FORM_RE = re.compile(r"^[a-zăâîșț]+(-[a-zăâîșț]+)*$")
...
if __name__ == "__main__":
    in_path = sys.argv[1] if len(sys.argv) > 1 else "ro-extract.jsonl.gz"
