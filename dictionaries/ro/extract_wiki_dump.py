# Extracts real word-frequency and bigram counts for Romanian from the COMPLETE Romanian Wikipedia XML dump
# (rowiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/cs/extract_wiki_dump.py. Romanian's own diacritic letters (ă/â/î/ș/ț) have no Turkish-style
# Unicode-casefolding quirk - plain `str.lower()` is correct here. Uses the modern comma-below ș/ț
# (U+0219/U+021B), confirmed directly to be this project's own Wiktionary source's standard (see
# extract_wiktionary.py's own module docstring).
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Fișier|Imagine):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in ROMANIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
