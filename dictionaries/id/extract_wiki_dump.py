# Extracts real word-frequency and bigram counts for Indonesian from the COMPLETE Indonesian Wikipedia XML
# dump (idwiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the
# same "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/ms/extract_wiki_dump.py (the same underlying alphabet, plain 26-letter Latin with no
# diacritics - confirmed directly against the real Wiktionary source).
...
DUMP = Path(
    ...
    "/scratchpad/idwiki/idwiki-full.xml.bz2"
)
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Berkas|Gambar):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in INDONESIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
