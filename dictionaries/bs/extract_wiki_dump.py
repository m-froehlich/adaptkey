# Extracts real word-frequency and bigram counts for Bosnian from the COMPLETE Bosnian Wikipedia XML dump
# (bswiki-latest-pages-articles.xml.bz2, the single combined dump - not a capped first split, per the same
# "full dump only" rule established for pt/it/nl/pl/tr). Directly modelled on
# dictionaries/hr/extract_wiki_dump.py. Bosnian's own diacritic letters (č/ć/đ/š/ž) have no Turkish-style
# Unicode-casefolding quirk - plain `str.lower()` is correct here. Like Croatian, Bosnian Wikipedia's own
# real-world usage is overwhelmingly Latin script (unlike Serbian - see dictionaries/sr/'s own module
# docstring for that separate, directly-measured decision).
...
DUMP = Path(
    ...
    "/scratchpad/bswiki/bswiki-full.xml.bz2"
)
BASE = Path(__file__).parent
FREQ_OUT = BASE / "wiki_dump_freq.tsv"
BIGRAM_OUT = BASE / "wiki_dump_bigram.tsv"

BOSNIAN_LETTERS = set("abcčćdđefghijklmnoprsštuvzž")
TOKEN_RE = re.compile(r"[a-bčćdđefghijklmnoprsštuvzž]+(?:-[a-bčćdđefghijklmnoprsštuvzž]+)*|['’]", re.IGNORECASE)
APOSTROPHES = ("'", "\u2019")
SINGLE_CHAR_OK = ("a", "i", "u", "o", "s")
...
FILE_IMAGE_RE = re.compile(r"\[\[(?:File|Datoteka|Slika):[^\]]*\]\]", re.IGNORECASE | re.DOTALL)
...
def process_page(raw_text):
    ...
    for raw_tok in TOKEN_RE.findall(cleaned.lower()):
        if raw_tok in APOSTROPHES:
            ...
            continue
        tok = raw_tok
        if any(ch not in BOSNIAN_LETTERS and ch != "-" for ch in tok):
            prev = None
            continue
